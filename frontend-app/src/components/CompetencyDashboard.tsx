      ...
      <div className="flex flex-col md:flex-row md:items-center gap-4 p-6">
        ...
        <div className="flex md:flex-col items-center justify-center md:w-24 shrink-0">
          <div className="h-16 w-16 rounded-xl bg-muted flex items-center justify-center">
            {React.cloneElement(categoryToIcon(competency.category), { className: 'h-8 w-8' })}
          </div>
          <Badge 
            variant="outline" 
            className={`${levelToColor(competency.level)} mt-2 hidden md:inline-flex`}
          >
            {competency.level}
          </Badge>
        </div>
        ...
        <div className="flex-grow min-w-0">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1 md:mt-2">
                ...
                <div className="flex items-center text-muted-foreground text-sm">
                  ...
                  <span>{competency.category.toLowerCase().replace('_', ' ')}</span>
                </div>
              </div>
            </div>
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem onClick={() => navigator.clipboard.writeText(competency.id)}>
                  Copy ID
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem asChild>
                  <Link href={`/competency/${competency.id}`} className="flex items-center">
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
            <div className="flex items-center">
              ...
              <span>{competency.behavioralIndicators?.length || 0} indicators</span>
            </div>
            <div className="flex items-center">
              <Calendar className="mr-1 h-4 w-4" />
              <span>{new Date(competency.lastModified).toLocaleDateString()}</span>
            </div>
...
function CompetencyCardGrid({
  ...
  const [sortBy, setSortBy] = useState<'name' | 'level' | 'indicators' | 'modified'>('name');
    ...
    return filtered.sort((a: Competency, b: Competency) => {
      ...
      switch (sortBy) {
        ...
        case 'level':
          ...
          break;
        case 'indicators':
          comparison = (a.behavioralIndicators?.length || 0) - (b.behavioralIndicators?.length || 0);
          break;
        case 'modified':
  ...
  const totalPages = Math.ceil(filteredAndSortedCompetencies.length / itemsPerPage);
      ...
      {currentItems.length > 0 ? (
        ...
        <Card className="p-12 text-center">
          <CardContent>
            ...
            <h3 className="mt-4 text-lg font-semibold">No competencies found</h3>
            <p className="mt-2 text-sm text-muted-foreground">
              {searchTerm 
                ? `No competencies match your search for "${searchTerm}"`
                : selectedCategory !== 'all' 
                ? `No competencies found in ${selectedCategory.replace('_', ' ')} category`
      ...
      {filteredAndSortedCompetencies.length > itemsPerPage && (
        <div className="flex items-center justify-between pt-4">
          <p className="text-sm text-muted-foreground">
            Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, filteredAndSortedCompetencies.length)} of {filteredAndSortedCompetencies.length} competencies
          </p>
...
const CompetencyDashboard: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const competenciesResponse = await fetch('http://localhost:8080/api/competencies', {
      ...
      const stats: DashboardStats = {
        ...
        totalBehavioralIndicators: competenciesData.reduce((sum, comp) => 
          sum + (comp.behavioralIndicators?.length || 0), 0),
      ...
      stats.averageIndicatorsPerCompetency = stats.totalBehavioralIndicators / stats.totalCompetencies;
      setStats(stats);
            ...
            <CardContent className="p-6">
              <div className="flex items-start gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-2xl font-bold tracking-tight">{stats.totalCompetencies}</p>
          ...
          <Card className="group hover:shadow-lg transition-all">
            <CardContent className="p-6">
              <div className="flex items-start gap-4">
                ...
                <div className="space-y-1.5">
                  <p className="text-sm font-medium text-muted-foreground">Behavioral Indicators</p>
                  <p className="text-2xl font-bold tracking-tight">{stats.totalBehavioralIndicators}</p>
          ...
          <Card className="group hover:shadow-lg transition-all">
            <CardContent className="p-6">
              <div className="flex items-start gap-4">
                ...
                <div className="space-y-1.5">
                  <p className="text-sm font-medium text-muted-foreground">Assessment Questions</p>
                  <p className="text-2xl font-bold tracking-tight">{stats.totalAssessmentQuestions}</p>
                  <Progress 
                    value={65} 
                    className="h-1 w-20" 
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex -space-x-1.5">
                    {Object.keys(stats.competenciesByCategory).slice(0, 4).map((category, i) => (
                      <div 
                        key={category}
                      ...
                      >
                        {React.cloneElement(categoryToIcon(category), { className: 'h-3 w-3' })}
                      </div>
                    ))}
