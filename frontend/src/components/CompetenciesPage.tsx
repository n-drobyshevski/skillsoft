...
// Main component
const CompetenciesPageNew: React.FC = () => {
...
const columns: ColumnDef<Competency>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const indicators = row.getValue("behavioralIndicators") as BehavioralIndicator[];
      return (
        <div className="flex items-center gap-2">
...
const proficiencyLevelToColor = (level: ProficiencyLevel): string => {
  const colors: { [key in ProficiencyLevel]: string } = {
    [ProficiencyLevel.NOVICE]: 'border-red-500/20 text-red-700 bg-red-50/90 dark:bg-red-950/90 dark:text-red-200 dark:border-red-400/30',
    [ProficiencyLevel.DEVELOPING]: 'border-amber-500/20 text-amber-700 bg-amber-50/90 dark:bg-amber-950/90 dark:text-amber-200 dark:border-amber-400/30',
    [ProficiencyLevel.PROFICIENT]: 'border-emerald-500/20 text-emerald-700 bg-emerald-50/90 dark:bg-emerald-950/90 dark:text-emerald-200 dark:border-emerald-400/30',
    [ProficiencyLevel.ADVANCED]: 'border-blue-500/20 text-blue-700 bg-blue-50/90 dark:bg-blue-950/90 dark:text-blue-200 dark:border-blue-400/30',
    [ProficiencyLevel.EXPERT]: 'border-violet-500/20 text-violet-700 bg-violet-50/90 dark:bg-violet-950/90 dark:text-violet-200 dark:border-violet-400/30'
  };
  return colors[level];
};
  ...
  // Stats component
const CompetencyStats: React.FC<{ competencies: Competency[] }> = ({ competencies }) => {
  const stats = React.useMemo(() => {
    const total = competencies.length;
    const active = competencies.filter(c => c.isActive).length;
    const totalIndicators = competencies.reduce((sum, c) => sum + (c.behavioralIndicators?.length || 0), 0);
    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
      <Card className="p-4 flex items-center space-x-4">
        ...
        <div>
          ...
          <div className="text-xs text-muted-foreground">Total Competencies</div>
        </div>
      </Card>
      <Card className="p-4 flex items-center space-x-4">
        <div className="p-3 bg-primary/10 rounded-lg">
        ...
        <div>
          ...
          <div className="text-xs text-muted-foreground">Active Competencies</div>
        </div>
      </Card>
      <Card className="p-4 flex items-center space-x-4">
        <div className="p-3 bg-primary/10 rounded-lg">
    ...
    <div className="container mx-auto px-4 py-8 space-y-8 w-full">
      ...
      {loading && (
        <div className="flex items-center justify-center py-12">
          ...
          <span className="ml-3 text-muted-foreground">Loading competencies...</span>
        </div>
      )}
...
export default CompetenciesPageNew;