...
public class TestAssemblerFactory {
    ...
    public TestAssemblerFactory(List<TestAssembler> assemblers) {
        this.assemblers = assemblers;
    }

    /**
     * Initialize assembler map on startup for thread safety.
     * Replaces lazy initialization to avoid race conditions.
     */
    @PostConstruct
    public void initializeAssemblers() {
        this.assemblerMap = assemblers.stream()
        ...
        log.info("Initialized {} test assemblers: {}",
            assemblerMap.size(), assemblerMap.keySet());
    }
    ...
    public TestAssembler getAssembler(TestBlueprintDto blueprint) {
        ...
        var assembler = assemblerMap.get(goal);
        if (assembler == null) {
            throw new IllegalArgumentException(
                "No assembler found for strategy: " + goal +
                ". Available strategies: " + assemblerMap.keySet()
            );
        }
    ...
    public boolean hasAssembler(AssessmentGoal goal) {
        return goal != null && assemblerMap.containsKey(goal);
    }
